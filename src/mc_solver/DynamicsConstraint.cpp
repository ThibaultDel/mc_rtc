...
void DynamicsConstraint::addToSolverImpl(QPSolver & solver)
{
  ...
  switch(backend_)
  {
    ...
    case QPSolver::Backend::TVM:
    {
      ...
      const auto& robot = solver.robot(robotIndex_);
      auto& robot_mb = robot.mb();
      auto mbc = robot.mbc();
      // const auto& actuated_joints = robot.module();
      rbd::Jacobian jac(robot_mb, robot_frame_name);
      solver.robot(robotIndex_).forwardKinematics(mbc);
      ...
      mc_rtc::log::info("Dim of qd = {} ", qd.size());
      mc_rtc::log::info("Dim of limits = {}", tvm_robot.limits().tu.size());
      ...
      for(std::vector<double> vec : qd){
        if(vec.size() == 0){
          zeros++;
        } else if (vec.size() == 1) {
          ones++;
          velocity.insert(velocity.end(),vec[0]);
          mc_rtc::log::info("inserted {} to the velocity vector", velocity[ones-1]);
        } else {
          mc_rtc::log::info("different size than 0 or 1");
        }
      }

      // 1. Specify the exact size based on the intermediate vector
      Eigen::VectorXd final_vector(velocity.size());

      // 2. Map the data directly (or copy)
      // The Map constructor is very efficient if you can use raw pointers:
      final_vector = Eigen::Map<Eigen::VectorXd>(
          velocity.data(), // Get the pointer to the underlying array
          velocity.size()
      );

      mc_rtc::log::info("We have {} ones and {} zeros", ones, zeros);
      mc_rtc::log::info("size of velocity vector is {}", velocity.size());
      // auto limit_left = (TL_multiplier*tvm_robot.limits().tu)/(c_res+1) - qd;

      // mc_rtc::log::info("Dim of limits = {}", limit_left->size());
      ...
      //                       tvm::task_dynamics::None(), {tvm::requirements::PriorityLevel(0)});
      // auto Mom_torque = problem.add(TL_multiplier*tvm_robot.limits().tl <= J_m*P_n*jac_f_w_f*tvm_robot.alphaD() <= (TL_multiplier*tvm_robot.limits().tu)/(c_res+1),
      //                       tvm::task_dynamics::None(), {tvm::requirements::PriorityLevel(0)});
      auto Mom_torque1 = problem.add((TL_multiplier*tvm_robot.limits().tl)/(c_res+1)-(J_m*P_n*jac_f_w_f*final_vector)/delta_t <= /* J_m*P_n*jac_f_w_f* */tvm::dot(tvm_robot.qJoints(),2) <= 
                                        (TL_multiplier*tvm_robot.limits().tu)/(c_res+1)-(J_m*P_n*jac_f_w_f*final_vector)/delta_t
                            , tvm::task_dynamics::None(), {tvm::requirements::PriorityLevel(0)});
      constraints_.push_back(Mom_torque1);
